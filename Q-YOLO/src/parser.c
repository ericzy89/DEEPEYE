...
{
    //
    int three = option_find_int(options, "three",0);
    int print_weights = option_find_int(options, "print_weights",0);
    int print_activations = option_find_int(options, "print_activations",0);
    int quantize = option_find_int(options, "quantize",0);
    //
    int n = option_find_int(options, "filters",1);
    int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
    ...
    convolutional_layer layer = make_convolutional_layer(batch,h,w,c,n,groups,size,stride,padding,activation, batch_normalize, binary, xnor, params.net->adam , three , print_weights , print_activations , quantize);
